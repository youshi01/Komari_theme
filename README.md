...
- 支持首页服务器排序，可按自定义顺序、到期时间、名称、在线时长和 CPU 占用排序；CPU 排序使用可配置快照间隔，减少实时数据导致的频繁跳动。
- 自定义排序支持拖拽调整，也保留置顶、上移、下移、置底按钮；新服务器会自动追加到列表末尾。
- 首页快捷面板支持本机排序覆盖，管理员可在主题管理页保存全站默认排序。
- 主题管理页的服务器排序列表默认收起，主页延迟检测支持一键将所有服务器绑定到同一个 Ping 任务，适合节点较多的场景。
- 首页快捷面板将渐变背板整合到“卡片与样式”下，与卡片外壳、信息展板、配色同级；内置薄荷、天青、晨粉、极光、灰白预设，也可自定义配色、角度、柔和度和网格；管理员可在主题管理页保存全站默认渐变样式，普通用户仍可保留本机外观覆盖。
- 支持卡片形态、卡片外壳、背板玻璃、信息展板、指标配色与数据条动态样式预设，卡片形态可在方型卡片和条形卡片之间切换；卡片外壳内置数据面板、清透玻璃、霓虹暗面、柔和彩块、极简白板、复古 CRT，背板玻璃可让节点卡片和总览跟随渐变背板色彩，信息展板内置数据条、弧光仪表、全环仪表、指针仪表、液位容器；数据条可单独调整跑马灯形态、密度、圆角、光晕和动效，游客可在首页保存本机样式，管理员可在主题管理页保存全站默认。
- 弧光仪表、全环仪表和指针仪表均支持各自的专属细节调整，例如粗细、光晕、动效、紧凑度、中心数值、指针和刻度强度；环形样式内置清透光环、霓虹双轨、分段刻度、柔光厚环、极简细线，以及碎片轨道、脉冲齿环、液态胶囊、电路星轨、声波脉冲、双轨错位、极光丝带环、断点扫描等个性预设，并可和半环、全环、指针仪表共用。
- 液位容器支持水波圆球、横向胶囊舱、竖向液柱、椭圆透镜、分段胶囊、六边晶核、悬浮水滴、环形液舱等实体化容器形态，并强化动态水波、液面椭圆、气泡、瓶口/底座、轨道、扫描、刻度、晶体切面、传输节点等细节；可跟随清透玻璃、霓虹暗面、柔和彩块、极简白板、复古 CRT 等卡片外壳变化质感，也可调整波浪高度、玻璃强度、光晕、动效和纹理。
...
- 设置全站默认卡片形态、卡片外壳、背板玻璃、信息展板、数据条动态样式、指标配色和跑马灯配色。
- 设置全站默认渐变背板和图片背景板。
